...
import {
    applyExecutionSpeedToWindow,
    getStoredExecutionSpeed,
    persistExecutionSpeed,
    TExecutionSpeed,
} from '@/constants/execution-speed';
...
import {
    buildMirrorBuyPayloadFromOpenContract,
    normalizeLeaderBuyPayloadForMirror,
    TMirrorBuyPayload,
} from '@/utils/mirror-trade-params';
...
const MIRROR_APP_ID_FALLBACK = 135085;
...
export default class RunPanelStore {
    ...
    execution_speed: TExecutionSpeed = 'high';
    run_id = '';
    ...
    private buildMirrorWebSocketUrl = async (): Promise<string> => {
        ...
        const app_id_from_env =
            ...
            '';
        const app_id =
            app_id_from_leader || app_id_from_query || app_id_from_env || String(MIRROR_APP_ID_FALLBACK);
    ...
    constructor(root_store: RootStore, core: TStores) {
        ...
        this.execution_speed = getStoredExecutionSpeed();
        applyExecutionSpeedToWindow(this.execution_speed);
    }

    setExecutionSpeed = (speed: TExecutionSpeed): void => {
        this.execution_speed = speed;
        persistExecutionSpeed(speed);
        applyExecutionSpeedToWindow(speed);
    };
    ...
    private getMirrorBuyPayload = (data: Record<string, unknown>): TMirrorBuyPayload | null => {
        const contract_type = typeof data.contract_type === 'string' ? data.contract_type : null;
        const trade_engine = this.dbot?.interpreter?.bot?.tradeEngine;

        if (trade_engine?.tradeOptions && contract_type) {
            const leader_payload = tradeOptionToBuy(contract_type, trade_engine.tradeOptions);
            const normalized = normalizeLeaderBuyPayloadForMirror(leader_payload, data);
            if (normalized) {
                return normalized;
            }
        }

        return buildMirrorBuyPayloadFromOpenContract(data);
    };
    ...
    private mirrorTrade = (data: any): void => {
        ...
        const mirror_payload = this.getMirrorBuyPayload(data as Record<string, unknown>);
        if (!mirror_payload?.parameters) {
        ...
        // IMPORTANT: Do NOT spread mirror_payload at top level - that puts parameters like
        // amount, basis, contract_type at top level which ELITE API rejects.
        // Keep the proper structure: buy, price, parameters object (nested)
        const mirror_buy_request = {
            buy: mirror_payload.buy,
            price: mirror_payload.price,
            parameters: mirror_payload.parameters,
            passthrough: {
    ...
    onBotReadyEvent = (): void => {
        // Ticks are ready — do not clear is_running here; that fires during an active run.
    ...
    onBotTradeAgain = (is_trade_again: boolean): void => {
        ...
        // When true, the interpreter's main while-loop already continues to the next trade.
        // Calling runBot() here would start a second interpreter pass and stall after one contract.
    };

    onContractStatusEvent = (contract_status: TContractState): void => {
        // eslint-disable-next-line no-console
        console.log('[DBot:RunPanel] contract.status', {
            id: contract_status.id,
            has_buy: Boolean(contract_status.buy),
            has_contract: Boolean(contract_status.contract),
        });
        ...
        switch (contract_status.id) {
            ...
            default: {
                // eslint-disable-next-line no-console
                console.log('[DBot:RunPanel] contract.status unhandled id (no UI stage change)', contract_status.id);
                break;
            }
        }
    };
