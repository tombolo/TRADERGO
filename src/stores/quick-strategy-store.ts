...
export default class QuickStrategyStore implements IQuickStrategyStore {
    ...
    onSubmit = async (data: TFormData) => {
        ...
        // Start only after the workspace import finishes. Subscribing to BLOCK_CREATE before `load()`
        // resolves could fire `onRunButtonClick` while Blockly/workspace helpers are still settling, which
        // led to a single completed trade cycle and then an exit (e.g. AI scanner + quick strategy path).
        if (action === 'RUN' && workspace && !load_result?.error) {
