...
import ExecutionSpeedToggle from './execution-speed-toggle';
...
const TradeAnimation = observer(({ className, should_show_overlay }: TTradeAnimation) => {
    ...
    const [speed_changed_pulse, setSpeedChangedPulse] = React.useState(false);
    const prev_execution_speed = React.useRef(execution_speed);
    ...
    React.useEffect(() => {
        if (prev_execution_speed.current === execution_speed) return;

        prev_execution_speed.current = execution_speed;
        setSpeedChangedPulse(true);
        const timeout_id = window.setTimeout(() => setSpeedChangedPulse(false), 700);
    ...
    const run_button = should_show_tooltip ? (
    ...
    return (
        <div className={classNames('animation__wrapper', className)}>
            <div className='animation__controls'>
                {run_button}
                <ExecutionSpeedToggle disabled={is_stop_button_visible} />
            </div>
            <div
                className={classNames('animation__container', className, {
                    ...
                    'animation--speed-fast': execution_speed === 'high',
                    'animation--speed-slow': execution_speed === 'slow',
                    'animation--speed-changed': speed_changed_pulse,
                })}
            >
