...
const Transactions = observer(({ is_drawer_open }: TTransactions) => {
    ...
    const getTransactionRowKey = (row: (typeof transaction_list)[number], index: number) => {
        switch (row.type) {
            case transaction_elements.CONTRACT:
    ...
    const renderMobileTransactionList = () => (
        <ThemedScrollbars className='transactions__mobile-scroll' autohide={false}>
            {transaction_list?.map((row, index) => (
                <TransactionItem
                    key={getTransactionRowKey(row, index)}
                    row={row}
                    onClickTransaction={onClickTransaction}
                    active_transaction_id={active_transaction_id}
                />
