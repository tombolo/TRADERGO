 ...
 * 2. For ELITE: return URL with app_id=135085 for ELITE OAuth flow
...
export const getSocketURL = async (): Promise<string> => {
    try {
        ...
        if (authSystem === 'ELITE') {
            ...
            const appId = (process.env.APP_ID || '135085').trim() || '135085';
            const eliteUrl = `wss://${host}/websockets/v3?app_id=${encodeURIComponent(appId)}`;

            console.log(
                '%c🔐 [getSocketURL] ELITE account detected - using legacy websocket route',
                'color: #FF6B00; font-weight: bold;',
                { host, appId }
            );
            console.log('%c📍 [getSocketURL] ELITE WebSocket URL:', 'color: #FF6B00;', eliteUrl);
            return eliteUrl;
        }
        ...
        const { isSpecialCaseLoginId, logSpecialAccountDebug } = await import('@/utils/account-helpers');
        if (isSpecialCaseLoginId(activeLoginId)) {
            logSpecialAccountDebug('getSocketURL_start', {
                activeLoginId,
                auth_system: authSystem,
                has_access_token: Boolean(authInfo.access_token),
            });
        }
...
export const validateCSRFToken = (token: string): boolean => {
    ...
    console.log('[validateCSRFToken] Validation Debug:', {
        ...
        receivedToken: token ? `${token.substring(0, 20)}...` : null,
        hasTimestamp: !!timestamp,
        tokensMatch: storedToken === token,
    });

    if (!storedToken || !timestamp) {
        console.error('[validateCSRFToken] ❌ Missing stored token or timestamp');
        return false;
    }

    // Check if token matches
    if (storedToken !== token) {
        console.error('[validateCSRFToken] ❌ Tokens do not match');
    ...
    console.log('[validateCSRFToken] Token age (ms):', tokenAge);
    ...
    console.log('[validateCSRFToken] ✅ Token is valid');
    return true;
};
...
export const generateOAuthURL = async (prompt?: string) => {
    try {
        ...
        if (hostname && clientId) {
            ...
            // DEBUG: Log the exact values being sent to Deriv
            console.log('%c🔐 OAuth Configuration', 'color: blue; font-weight: bold;', {
                environment,
                authEndpoint: hostname,
                clientId,
                redirectUrl,
                scopes,
            });
