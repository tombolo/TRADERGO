        ...
        async getTradingTimes(): Promise<any> {
            try {
                ...
                try {
                    ...
                    logger.warn('Trading times initialization failed, using fallback:', initError);
                    // Continue with fallback even if initialization fails
                }
                ...
                if (
                    ...
                    Object.keys(tradingTimesData).length > 0
                ) {
                    return transformTradingTimesData(tradingTimesData);
                }

                // Try to trigger setTradingTimes fallback manually for Elite accounts or when API fails
                ...
                if (typeof apiHelpers.trading_times.setTradingTimes === 'function') {
                    try {
                        ...
                        if (fallbackData && typeof fallbackData === 'object' && Object.keys(fallbackData).length > 0) {
                            logger.warn('Using fallback trading times data');
                            return transformTradingTimesData(fallbackData);
                        }
                    } catch (fallbackError) {
                        logger.warn('Fallback trading times retrieval failed:', fallbackError);
                    }
                }

                // Return empty object - chart will work without trading times
                logger.warn('No trading times data available - chart will function without trading times');
                return {};
            } catch (error) {
                ...
                // Return empty object instead of failing - allows chart to work
                return {};
            }
        },
    };
