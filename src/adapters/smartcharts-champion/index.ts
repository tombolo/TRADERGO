    ...
    const adapter: SmartchartsChampionAdapter = {
        ...
        async getChartData(): Promise<{ activeSymbols: ActiveSymbols; tradingTimes: TradingTimesMap }> {
            try {
                ...
                let tradingTimes = transformations.toTradingTimesMap(tradingTimesData);

                // Ensure all active symbols have trading times entries (with defaults if missing)
                // This prevents "Symbol not in _tradingTimesMap" errors for Elite accounts
                activeSymbols.forEach(symbol => {
                    if (!tradingTimes[symbol.symbol]) {
                        // Provide default trading times for symbols without data
                        tradingTimes[symbol.symbol] = {
                            isOpen: true,
                            openTime: '00:00',
                            closeTime: '23:59',
                        };
                        logger.warn(`No trading times for ${symbol.symbol}, using default`);
                    }
                });
