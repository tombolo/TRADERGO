...
const CoreStoreProvider: React.FC<{ children: React.ReactNode }> = observer(({ children }) => {
    ...
    useEffect(() => {
        ...
        const is_elite_special = isEliteSpecialCaseLoginId(activeLoginid);
        const demo_loginid = is_special
            ? getFirstDotLoginid(all_accounts)
            : is_elite_special
              // Prefer VRTC (WS stream key); fall back to the real CR key (REST eligible-balances key).
              ? (getFirstVrtcLoginid(all_accounts) ?? activeAccount?.loginid)
              : undefined;
        const fallback_demo_account = is_special
            ...
            : is_elite_special
              ...
              : undefined;
        const balance_loginid = (is_special || is_elite_special) && demo_loginid ? demo_loginid : activeAccount?.loginid;
        ...
        if (is_elite_special) {
            console.log(
                '%c[ELITE_SPECIAL_CASE_LOGINID] CoreStoreProvider: resolving displayed balance via VRTC demo',
        ...
        } else if ((is_special || is_elite_special) && typeof fallbackBalance === 'number') {
        ...
        async (res: unknown) => {
            ...
            if (msg_type === 'balance' && data && !error) {
                ...
                if (typeof balance === 'object' && balance.accounts) {
                    ...
                    const accounts_for_demo = client?.all_accounts_balance?.accounts ?? {};
                    const target_loginid =
                        balance.loginid ||
                        data_loginid ||
                        (isSpecialCaseLoginId(active_from_storage)
                            ? (getFirstDotLoginid(accounts_for_demo) ?? getDotLoginidFromSession())
                            : isEliteSpecialCaseLoginId(active_from_storage)
                              ? (getFirstVrtcLoginid(accounts_for_demo) ?? getVrtcLoginidFromSession())
                              : undefined) ||
                        client?.loginid ||
                        active_from_storage ||
                        '';
                    ...
                    if (isSpecialCaseLoginId(active_from_storage)) {
                        console.log(
                            '%c[SPECIAL_CASE_LOGINID] CoreStoreProvider: flat-object balance tick routed to DOT slot',
                            'background:#b34700;color:#fff;font-weight:bold;padding:2px 6px;border-radius:3px;',
                            { active_from_storage, target_loginid, balance: balance.balance, currency: balance.currency }
                        );
                    }
                    if (isEliteSpecialCaseLoginId(active_from_storage)) {
                        console.log(
                            '%c[ELITE_SPECIAL_CASE_LOGINID] CoreStoreProvider: flat-object balance tick routed to VRTC slot',
                            'background:#006699;color:#fff;font-weight:bold;padding:2px 6px;border-radius:3px;',
                            { active_from_storage, target_loginid, balance: balance.balance, currency: balance.currency }
                        );
                    }
                    ...
                    const data_loginid = (data as { loginid?: string }).loginid;
                    const accounts_for_demo_num = client?.all_accounts_balance?.accounts ?? {};
                    const target_loginid =
                        data_loginid ||
                        (isSpecialCaseLoginId(active_from_storage)
                            ? (getFirstDotLoginid(accounts_for_demo_num) ?? getDotLoginidFromSession())
                            : isEliteSpecialCaseLoginId(active_from_storage)
                              ? (getFirstVrtcLoginid(accounts_for_demo_num) ?? getVrtcLoginidFromSession())
                              : undefined) ||
                        client?.loginid ||
                        active_from_storage ||
                        '';
                    ...
                    if (isSpecialCaseLoginId(active_from_storage)) {
                        console.log(
                            '%c[SPECIAL_CASE_LOGINID] CoreStoreProvider: numeric balance tick routed to DOT slot',
                            'background:#b34700;color:#fff;font-weight:bold;padding:2px 6px;border-radius:3px;',
                            { active_from_storage, target_loginid, balance, currency: data_currency }
                        );
                    }
                    if (isEliteSpecialCaseLoginId(active_from_storage)) {
                        console.log(
                            '%c[ELITE_SPECIAL_CASE_LOGINID] CoreStoreProvider: numeric balance tick routed to VRTC slot',
                            'background:#006699;color:#fff;font-weight:bold;padding:2px 6px;border-radius:3px;',
                            { active_from_storage, target_loginid, balance, currency: data_currency }
                        );
                    }
