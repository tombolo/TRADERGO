...
class DBot {
     ...
     */
    runBot() {
        if (api_base.is_stopping || this.is_bot_running) return;

        try {
            api_base.is_stopping = false;
            const code = this.generateCode();
            if (!this.interpreter?.bot?.tradeEngine?.checkTicksPromiseExists()) {
                // Orphaned api_base.onMessage listeners (OpenContract, Proposal, ticks, …) from the previous
                // Interpreter otherwise stay registered and see every proposal_open_contract — with no
                // contractId they look like "mismatches" and can interfere with debugging and behaviour.
    ...
    generateCode(limitations = {}) {
        return `
            ...
            // Default after_purchase function if user didn't define one - continue trading
            if (!BinaryBotPrivateAfterPurchase) {
                BinaryBotPrivateAfterPurchase = function() { return true; };
            }
