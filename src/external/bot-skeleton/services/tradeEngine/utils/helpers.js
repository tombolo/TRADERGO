...
export const tradeOptionToProposal = (trade_option, purchase_reference) => {
    ...
    return trade_option.contractTypes.map(type => {
        ...
        // ELITE uses 'symbol', ZOOM uses 'underlying_symbol'
        if (isElite) {
...
export const tradeOptionToBuy = (contract_type, trade_option) => {
    ...
    // Build base parameters that both ELITE and ZOOM need
    const baseParams = {
        amount: trade_option.amount,
        basis: trade_option.basis,
        contract_type,
    ...
    if (trade_option.prediction !== undefined) {
        baseParams.selected_tick = trade_option.prediction;
    }
    if (!['TICKLOW', 'TICKHIGH'].includes(contract_type) && trade_option.prediction !== undefined) {
        baseParams.barrier = trade_option.prediction;
    } else if (trade_option.barrierOffset !== undefined) {
        baseParams.barrier = trade_option.barrierOffset;
    }
    if (trade_option.secondBarrierOffset !== undefined) {
        baseParams.barrier2 = trade_option.secondBarrierOffset;
    }
    if (!isEmptyObject(trade_option.app_markup_percentage)) {
        baseParams.app_markup_percentage = trade_option.app_markup_percentage;
    }
    if (!isEmptyObject(trade_option.barrier_range)) {
        baseParams.barrier_range = trade_option.barrier_range;
    }
    if (!isEmptyObject(trade_option.date_expiry)) {
        baseParams.date_expiry = trade_option.date_expiry;
    }
    if (!isEmptyObject(trade_option.date_start)) {
        baseParams.date_start = trade_option.date_start;
    }
    if (!isEmptyObject(trade_option.product_type)) {
        baseParams.product_type = trade_option.product_type;
    }
    if (!isEmptyObject(trade_option.trading_period_start)) {
        baseParams.trading_period_start = trade_option.trading_period_start;
    }
    if (!isEmptyObject(trade_option.limit_order)) {
        baseParams.limit_order = trade_option.limit_order;
    }
    if (['MULTUP', 'MULTDOWN'].includes(contract_type)) {
        baseParams.duration = undefined;
        baseParams.duration_unit = undefined;
        baseParams.multiplier = trade_option.multiplier;
    }
    if (['ACCU'].includes(contract_type)) {
        baseParams.duration = undefined;
        baseParams.duration_unit = undefined;
        baseParams.growth_rate = trade_option.growth_rate;
    }

    // ZOOM API expects parameters wrapped, ELITE only accepts proposal ID and price
    if (isElite) {
        return {
            buy: '1',
...
export const createDetails = contract => {
    const { sell_price: sellPrice, bid_price: bidPrice, buy_price: buyPrice, currency, exit_tick, is_expired } = contract;
    // Prefer sell_price (settled), then bid_price (live/ELITE API), then exit_tick for expired contracts
    const resolvedSellPrice = sellPrice !== undefined ? sellPrice : bidPrice;
    const exitPrice =
        resolvedSellPrice !== undefined ? resolvedSellPrice : is_expired && exit_tick !== undefined ? exit_tick : buyPrice;
    const profit = getRoundedNumber(exitPrice - buyPrice, currency);
    const result = profit < 0 ? 'loss' : 'win';

    return [
        contract.transaction_ids.buy,
        +contract.buy_price,
        exitPrice, // Use calculated exit price instead of sell_price (which may be undefined for expired contracts)
        profit,
