...
export default Engine =>
    class Total extends Engine {
        ...
        updateTotals(contract) {
            // ELITE API may use different field names than ZOOM
            // Contract response can have different structures:
            // - sell_price + buy_price (traditional)
            // - bid_price + buy_price (live update)
            // - payout + stake (ELITE alternative)
            // - profit field directly (some APIs)
            ...
            // Try sell_price method (traditional) - most reliable
            if ((sell_price || bid_price) && buyPrice) {
                sellPrice = sell_price || bid_price;
                profit = getRoundedNumber(Number(sellPrice) - Number(buyPrice), currency);
                console.log('[DBot:Total] Calculated profit from sell_price/bid_price:', {
                    sellPrice,
                    buyPrice,
                    profit,
                });
            }
            // Try payout method (ELITE alternative)
            else if (payout !== undefined && stake !== undefined) {
                sellPrice = payout;
                profit = getRoundedNumber(Number(payout) - Number(stake), currency);
                console.log('[DBot:Total] Calculated profit from payout/stake:', { payout, stake, profit, sellPrice });
            }
            // Error case - missing price data
            else {
                console.error('[DBot:Total] Cannot calculate profit - missing required fields:', {
                    sell_price,
                    bid_price,
                    buyPrice,
                    payout,
        ...
        validateTradeOptions(tradeOptions) {
            ...
            if (take_profit) {
                if (!tradeOptions.limit_order) tradeOptions.limit_order = {};
