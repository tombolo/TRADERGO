...
const PREDICTION_SCANNER_MS = 1100;
...
export const SpeedLabPanel = ({ onClose }: TSpeedLabPanelProps) => {
    ...
    const [prediction_scan_state, setPredictionScanState] = React.useState<'idle' | 'scanning' | 'ready'>('idle');
    ...
    const prediction_focus_label = React.useMemo(() => {
        if (prediction_mode === 'selected_tick') return `Tick ${trade_prediction}`;
        if (prediction_mode !== 'barrier_digit') return '';
        if (contract_type_api === 'DIGITOVER') return `Over ${trade_prediction}`;
        if (contract_type_api === 'DIGITUNDER') return `Under ${trade_prediction}`;
        if (contract_type_api === 'DIGITMATCH') return `Match ${trade_prediction}`;
        if (contract_type_api === 'DIGITDIFF') return `Differ from ${trade_prediction}`;
        return `Digit ${trade_prediction}`;
    }, [contract_type_api, prediction_mode, trade_prediction]);

    React.useEffect(() => {
        if (!prediction_mode) {
            setPredictionScanState('idle');
            return;
        }
        setPredictionScanState('scanning');
        const timer = window.setTimeout(() => {
            setPredictionScanState('ready');
        }, PREDICTION_SCANNER_MS);
        return () => window.clearTimeout(timer);
    }, [prediction_mode, trade_prediction, contract_type_api]);
        ...
        <div className='speed-lab'>
            ...
            <div className='speed-lab__body'>
                ...
                <section className='speed-lab__section' aria-labelledby='speed-lab-s3'>
                    ...
                    {prediction_mode && (
                        <div
                            className={classNames('speed-lab__prediction-scanner', {
                                'speed-lab__prediction-scanner--scanning': prediction_scan_state === 'scanning',
                                'speed-lab__prediction-scanner--ready': prediction_scan_state === 'ready',
                            })}
                            role='status'
                            aria-live='polite'
                        >
                            <div className='speed-lab__prediction-scanner-head'>
                                <span className='speed-lab__prediction-scanner-chip'>
                                    <Localize i18n_default_text='Prediction scanner' />
                                </span>
                                <span className='speed-lab__prediction-scanner-meta'>
                                    {prediction_scan_state === 'scanning' ? (
                                        <Localize i18n_default_text='Scanning best signal...' />
                                    ) : (
                                        <Localize i18n_default_text='Signal locked' />
                                    )}
                                </span>
                            </div>
                            <div className='speed-lab__prediction-scan-track' aria-hidden>
                                <span className='speed-lab__prediction-scan-beam' />
                            </div>
                            <p className='speed-lab__prediction-scanner-output'>
                                {prediction_scan_state === 'scanning' ? (
                                    <Localize i18n_default_text='Checking momentum, payout edge, and live tick flow for your selected setup...' />
                                ) : (
                                    <Localize
                                        i18n_default_text='Best prediction candidate: {{focus_label}}'
                                        values={{ focus_label: prediction_focus_label }}
                                    />
                                )}
                            </p>
                        </div>
