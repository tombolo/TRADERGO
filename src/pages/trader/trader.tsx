...
import { getDotLoginidFromSession, getFirstDotLoginid, getFirstVrtcLoginid, getVrtcLoginidFromSession, isEliteSpecialCaseLoginId, isSpecialCaseLoginId } from '@/utils/account-helpers';
import './trader.scss';
...
/**
 * Detect user account type: ELITE or ZOOM
 * Based on active_loginid prefix:
 * - CR or VRTC prefix → ELITE
 * - Otherwise → ZOOM
 */
function detectAccountType(): 'ELITE' | 'ZOOM' {
    const activeLoginId = localStorage.getItem('active_loginid');

    if (!activeLoginId) {
        console.log('[DTrader] Account type: ZOOM (default - no active_loginid)');
        return 'ZOOM';
    }

    // Check if login ID starts with CR or VRTC
    if (activeLoginId.startsWith('CR') || activeLoginId.startsWith('VRTC')) {
        console.log(`[DTrader] Account type: ELITE (loginid: ${activeLoginId} starts with CR/VRTC)`);
        return 'ELITE';
    }

    // All other login IDs are ZOOM
    console.log(`[DTrader] Account type: ZOOM (loginid: ${activeLoginId})`);
    return 'ZOOM';
}
...
function resolveDTraderEmbedCredentials(): { loginId?: string; accessToken?: string } {
    ...
    const eliteSpecialActive = Boolean(activeLoginId && isEliteSpecialCaseLoginId(activeLoginId));

    const dotLoginId = getFirstDotLoginid(accountsList) ?? getDotLoginidFromSession();
    const vrtcLoginId = getFirstVrtcLoginid(accountsList) ?? getVrtcLoginidFromSession();
    const preferredLoginId = specialActive ? dotLoginId : eliteSpecialActive ? vrtcLoginId : activeLoginId;

    if (specialActive) {
        console.log(
            '%c[SPECIAL_CASE_LOGINID] resolveDTraderEmbedCredentials: ROT account detected — using paired DOT wallet for DTrader embed',
            'background:#b34700;color:#fff;font-weight:bold;padding:2px 6px;border-radius:3px;',
            { activeLoginId, dotLoginId, preferredLoginId, accountsCount: Object.keys(accountsList).length }
        );
    }
    if (eliteSpecialActive) {
        console.log(
            '%c[ELITE_SPECIAL_CASE_LOGINID] resolveDTraderEmbedCredentials: CR real account detected — using paired VRTC demo for DTrader embed',
            'background:#006699;color:#fff;font-weight:bold;padding:2px 6px;border-radius:3px;',
            { activeLoginId, vrtcLoginId, preferredLoginId, accountsCount: Object.keys(accountsList).length }
        );
    }
    ...
    if (eliteSpecialActive) {
        const vrtcPair = Object.entries(accountsList).find(
            ([id, token]) => id.startsWith('VRTC') && typeof token === 'string' && token.length > 0
        );
        if (vrtcPair) {
            return { loginId: vrtcPair[0], accessToken: vrtcPair[1] };
        }
    }
    ...
    const firstPair = Object.entries(accountsList).find(
        ([id, token]) =>
            ...
            !(eliteSpecialActive && isEliteSpecialCaseLoginId(id))
    );
    if (firstPair) {
    ...
    if (oauthToken) {
        const loginForEmbed = specialActive ? dotLoginId : eliteSpecialActive ? vrtcLoginId : activeLoginId;
        if (loginForEmbed && !isSpecialCaseLoginId(loginForEmbed) && !isEliteSpecialCaseLoginId(loginForEmbed)) {
        ...
        try {
            ...
            const id = specialActive
                ...
                : eliteSpecialActive
                  ? accounts.find(a => a.account_id?.startsWith('VRTC'))?.account_id
                  : accounts.find(a => a.account_id)?.account_id;
            if (id && !isSpecialCaseLoginId(id) && !isEliteSpecialCaseLoginId(id)) {
    ...
    if (legacy) {
        const loginForLegacy = specialActive ? dotLoginId : eliteSpecialActive ? vrtcLoginId : activeLoginId;
        if (loginForLegacy && !isSpecialCaseLoginId(loginForLegacy) && !isEliteSpecialCaseLoginId(loginForLegacy)) {
...
const DTraderAutoLogin: React.FC<DTraderAutoLoginProps> = ({
    ...
    const [accountType, setAccountType] = useState<'ELITE' | 'ZOOM'>('ZOOM');
    const [isLoading, setIsLoading] = useState(true);
    const [error, setError] = useState<string | null>(null);
    const authCheckInterval = useRef<ReturnType<typeof setInterval> | null>(null);

    const buildIframeUrl = useCallback(
        (accessToken?: string, loginId?: string, type?: 'ELITE' | 'ZOOM') => {
            // Select appropriate URL based on account type
            const selectedUrl = type === 'ELITE' ? eliteDtraderUrl : dtraderUrl;

            let embedBase: string;
            try {
                embedBase = normalizeDTraderEmbedBaseUrl(selectedUrl);
            } catch {
            ...
            try {
                ...
                // Debug: log full iframe URL + all passed params/account details
                try {
                    const u = new URL(url);
                    const params = Object.fromEntries(u.searchParams.entries());
                    console.log('[DTrader iframe] params passed to iframe', {
                        iframeUrl: url,
                        embedBase,
        ...
        [appId, defaultSymbol, dtraderUrl, eliteDtraderUrl]
    );
    ...
    const checkAuthAndUpdate = useCallback(() => {
        try {
            ...
            buildIframeUrl(accessToken, loginId, type);
        } catch (err) {
        ...
        >
            <iframe
                ...
                style={{
                    ...
                    minHeight: 0,
                    ...(accountType === 'ELITE' && { marginTop: '-68px', marginBottom: '50px' }),
                }}
