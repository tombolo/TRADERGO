...
const updateBrandColorsInThemes = () => {
    ...
    brandColorLines.push('');
    brandColorLines.push('    --hub-leaf: #2f855a;');
    ...
    if (brandCommentStart !== -1) {
        ...
        if (aiEndMarker !== -1) {
            ...
            /* Always end before app-level tokens — avoid matching duplicate inner `/*` blocks inside brand vars */
            const appCardsMarker = themesContent.indexOf('\n    // App Cards', brandCommentStart);
            if (appCardsMarker !== -1) {
                endPoint = appCardsMarker;
            } else {
